...
/**
 * Internal quicksort method that makes recursive calls.
 * Uses median partitioning and a cutoff of 10.
 * a is an array of Comparable items.
 * left is the left-most index of the subarray.
 * right is the right-most index of the subarray.
 */
template <typename Comparable, typename Comparator>
void quicksortHelper2( vector<Comparable> & a, int left, int right, Comparator less_than)
{
  if( left + 10 <= right )
  {
    ...
    quicksortHelper2( a, left, i - 1, less_than );     // Sort small elements
    quicksortHelper2( a, i + 1, right, less_than);    // Sort large elements
  }
  else  // Do an insertion sort on the subarray
    insertionSort( a, left, right, less_than);
}
 ...
 */
template <typename Comparable>
const Comparable & first_elem( vector<Comparable> & a, int left, int right ) {
 ...
 */
template <typename Comparable, typename Comparator>
void quicksortHelper3( vector<Comparable> & a, int left, int right, Comparator less_than)
{
  if( left + 10 <= right )
  {
    const Comparable & pivot = first_elem( a, left, right );
    ...
    quicksortHelper3( a, left, i - 1, less_than );     // Sort small elements
    quicksortHelper3( a, i + 1, right, less_than);    // Sort large elements
  }
  else  // Do an insertion sort on the subarray
    insertionSort( a, left, right, less_than);
}
...
int main(int argc, char **argv) {
  ...
  if(comparison_type == "less") {
    ...
    auto end = chrono::high_resolution_clock::now();    
    cout << chrono::duration_cast<chrono::nanoseconds>(end - begin).count() << " ns" << endl;
    std::cout << VerifyOrder(input_vector, less<int>{}) << std::endl;
